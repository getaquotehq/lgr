...
export default {
  async fetch(
    ...
    if (url.pathname === "/" || url.pathname === "/health") {
      return new Response(
        JSON.stringify({ status: "ok", service: "leadgenrentalshq-api" }),
        {
          status: 200,
    ...
    if (!url.pathname.startsWith("/v1/")) {
      return new Response(
        JSON.stringify({
          error: "Not Found",
          message: "API routes are available under /v1/. See https://leadgenrentalshq.com/api-docs for documentation.",
        }),
        {
          status: 404,
