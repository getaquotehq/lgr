...
```
leadgenrentals.com.au/api
```

Cloudflare will automatically provision the SSL certificate.

> **DNS requirement:** Your `leadgenrentals.com.au/dashboard` domain must already be on Cloudflare DNS (orange-clouded). If it's on another DNS provider, use a Workers Route + CNAME instead.

### 5. Verify

```bash
curl https://leadgenrentals.com.au/api/health
# → {"status":"ok","service":"leadgenrentalshq-api"}

curl https://leadgenrentals.com.au/api/v1/leads \
