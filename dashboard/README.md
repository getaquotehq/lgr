...
Ported from `ql-hq` (which does the same job for QuoteLeads) and rebranded. It
...
1. **Run the migrations.** 90 migrations came across in `../supabase/migrations/`
   (the `2026051*`-`2026072*` LeadGenRentalsHQ set). They create ~50 tables:
   companies, profiles, leads, conversations, ppl_orders, lead_disputes,
   sms_credits and the rest. None collide with the LGR rental tables - the one
   clash, `leads`, was resolved by renaming LGR's to `asset_leads`.

2. **Deploy the edge functions.** 34 came across into `../supabase/functions/`.
   The one name clash was `stripe-webhook`, kept as **`hq-stripe-webhook`** so
   it doesn't overwrite the rental checkout webhook.
...
- There is no lgr <-> dashboard sync layer yet. The QuoteLeads equivalents
